...
# CSV/txt file path
# file_name = "C:/TetherNet/Shubhrika/ICRA2025/results/combined_data.csv"

file_name = "C:/Users/hahaha/Desktop/shubhrik/UrsoNet-master/datasets/combined_data.csv"
...
class Tethernet_Env(gym.Env):
    def __init__(self, port, env_id, num_envs):
        ...
        row_number = self.get_random_row(self.env_id, self.iteration, self.num_envs, total_rows)
        
        self.target_ground_truth, self.target_states_est, self.target_angular_velocity = self.read_data(file_name, row_number)
        ...
        self.pose_lb = [-9, -9, 40, -180, -180, -180, -180, -180, -180, -180, -180, -180, -180, -180, -180]
        self.pose_ub = [9, 9, 60, 180, 180, 180, 180, 180, 180, 180, 180, 180, 180, 180, 180]
        ...
        # Bound the estimated states
        self.bounded_target_states_est = self.bound_states(self.target_states_est)
        # print(f'Bounded states : {self.bounded_target_states_est}')

        # Normalize estimated states
        self.states = self.normalize_states(self.bounded_target_states_est)
        # print(f'Current state: {self.states}')

    def get_random_row(self, env_id, iteration, num_envs, total_rows):

        seed = env_id * 1000 + iteration


        offset = random.randint(0, total_rows -1)

        row_number = (offset + env_id + iteration * num_envs) % total_rows
    ...
    def read_data(self, file_name, row_number):
       ...
       with open(file_name, mode='r', encoding='utf-8-sig') as file:  # The 'utf-8-sig' encoding automatically removes the BOM
        ...
        return ground_truth_pose, estimated_pose, angular_velocity
    ...
    def calculate_reward(self, full_traj, target_ground_truth_pose, target_states_est, action6):
        ...
        estimated_pose = target_states_est
        ...
        r_phase_MU = np.transpose(np.array([xMU[t_close, :], yMU[t_close, :], zMU[t_close, :]]))
        r_D = np.tile(CT, (4,1))
        r_rel = r_phase_MU - r_D

        Rphase = 0
        if r_rel[0, 0]<0 and r_rel[0, 0]<0:  # MU1
            Rphase = Rphase + 0.25
        if r_rel[1, 0]>0 and r_rel[1, 0]<0:  # MU2
            Rphase = Rphase + 0.25
        if r_rel[0, 0]<0 and r_rel[0, 0]>0:  # MU3
            Rphase = Rphase + 0.25
        if r_rel[0, 0]>0 and r_rel[0, 0]>0:  # MU4
            Rphase = Rphase + 0.25
        ...
        # Write a newline character to the file
            act_rw =  np.asarray([Rcenter, Rfuel, Rarea, Ropen, Rphase, total_reward, 
                                  ...
                                  estimated_pose[0], estimated_pose[1], estimated_pose[2], estimated_pose[3], estimated_pose[4], estimated_pose[5] ])
        ...
        # Write a newline character to the file
                act_rw =  np.concatenate([Anet, Mfuel, beta_arr[-1]] )
                line_act_rw = " ".join(act_rw.astype(str)) + "\n"
                f.write(line_act_rw)
                # time.sleep(2)
        ...
        #         act_rw = np.asarray([v_net_list[i][0], v_net_list[i][1], v_net_list[i][2], direction_vector_list[i][0], direction_vector_list[i][1], direction_vector_list[i][2], cos_beta_list[i], beta_arr_list[i]])
        #         line_act_rw = " ".join(act_rw.astype(str)) + "\n"
        #         f.write(line_act_rw)
        #         # time.sleep(2)
    ...
    #### DEBUG AREA #####################
    # def calculate_net_area(self, open_traj_flatten):
    
    # # [MU1_x, MU1_y, MU1_z, MU2_x, MU2_y, MU2_z, MU3_x, MU3_y, MU3_z, MU4_x, MU4_y, MU4_z]

    #     # Extract positions of each MU
    #     MU1 = np.array([open_traj_flatten[0], open_traj_flatten[1], open_traj_flatten[2]])  # [MU1_x, MU1_y, MU1_z]
    #     MU2 = np.array([open_traj_flatten[3], open_traj_flatten[4], open_traj_flatten[5]])  # [MU2_x, MU2_y, MU2_z]
    #     MU3 = np.array([open_traj_flatten[6], open_traj_flatten[7], open_traj_flatten[8]])  # [MU3_x, MU3_y, MU3_z]
    #     MU4 = np.array([open_traj_flatten[9], open_traj_flatten[10], open_traj_flatten[11]])  # [MU4_x, MU4_y, MU4_z]

    #    
        
    #     # Triangle MU1-MU2-MU3
    #     a12 = np.linalg.norm(MU2 - MU1)  # Distance between MU1 and MU2
    #     b23 = np.linalg.norm(MU3 - MU2)  # Distance between MU2 and MU3
    #     c31 = np.linalg.norm(MU1 - MU3)  # Distance between MU1 and MU3
    #     s1 = (a12 + b23 + c31) / 2  # Semi-perimeter
    #     Area1 = np.sqrt(max(0, s1 * (s1 - a12) * (s1 - b23) * (s1 - c31)))  # Heron's formula for area

    #     # Triangle MU1-MU3-MU4
    #     a13 = np.linalg.norm(MU3 - MU1)  # Distance between MU1 and MU3
    #     b34 = np.linalg.norm(MU4 - MU3)  # Distance between MU3 and MU4
    #     c41 = np.linalg.norm(MU4 - MU1)  # Distance between MU1 and MU4
    #     s2 = (a13 + b34 + c41) / 2  # Semi-perimeter
    #     Area2 = np.sqrt(max(0, s2 * (s2 - a13) * (s2 - b34) * (s2 - c41)))  # Heron's formula for area
    ...
    def step(self, action):
       ...
       # convert GT to list
        ...
        # Calculate reward
        reward = self.calculate_reward(full_traj, self.target_ground_truth, self.target_states_est, action6)
        ...
        with open('./tethernet_Experiment/action_reward_log%.3d.txt' % self.port, 'a') as f:
            act_rw = np.append(target_ground_list, surrogate_input_array + [reward])
            line_act_rw = " ".join(act_rw.astype(str)) + "\n"
            f.write(line_act_rw)

        pickle_folder = "Pickle_folder"
        pickle_file = f"./tethernet_Experiment/{pickle_folder}/simoutput_log{self.port}.pkl"
    ...
    def reset(self):
        ...
        row_number = self.iteration * self.num_envs + self.env_id
        print(f"Env {self.env_id} resetting, reading row {row_number} from CSV")

        self.target_ground_truth, self.target_states_est, self.target_angular_velocity = self.read_data(file_name, row_number)

        # print(f"Coordinates sent to env {self.port} : {self.target_ground_truth}")

        # Estimated Pose
        # print(f'Estimated pose : {self.target_states_est}')

        # Bound State
        target_states_est = self.bound_states(self.target_states_est)
        # print('Bounded states: ', target_states_est)

        # Normalize states
        if isinstance(target_states_est, (tuple, list)):
            states = self.normalize_states(target_states_est)
            # print(f'Normalized states space values: {self.states}')
        else:
            raise TypeError(f"Expected target_states_est to be a list or tuple, got {type(target_states_est)}")

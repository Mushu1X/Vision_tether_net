...
############### Binary classifier ####################
class MLP_DT_Complex2(nn.Module):
    def __init__(self, input_size, output_size):
        super(MLP_DT_Complex2, self).__init__()
        # Adding more layers and more units in each layer can help model complex non-linearities
        self.fc1 = nn.Linear(input_size, 256)  # Increase the size of the first hidden layer
        self.tanh1 = nn.Tanh()
        self.dropout1 = nn.Dropout(p=0.5)  # Add dropout for regularization
        self.fc2 = nn.Linear(256, 512)  # Further increase for the second layer
        self.tanh2 = nn.Tanh()
        self.dropout2 = nn.Dropout(p=0.5)  # Add dropout for regularization
        self.fc3 = nn.Linear(512, 1024)  # Additional layer
        self.dropout3 = nn.Dropout(p=0.5)
        self.tanh3 = nn.Tanh()
        self.fc4 = nn.Linear(1024, 512)  # Output layer
        self.dropout4 = nn.Dropout(p=0.5)
        self.tanh4 = nn.Tanh()
        self.fc5 = nn.Linear(512, 256)  # Output layer
        self.dropout5 = nn.Dropout(p=0.5)
        self.tanh5 = nn.Tanh()
        self.fc6 = nn.Linear(256, 128)
        self.dropout6 = nn.Dropout(p=0.5)
        self.tanh6 = nn.Tanh()
        self.fc7 = nn.Linear(128, output_size) # Output layer
...
def run_MLP(model_inputs):
    ...
    numMu = 4
    input_cols = ["Tar_X", "Tar_Y", "Tar_Z", "W_X", "W_Y", "W_Z", "O_X", "O_Y", "O_Z"]

    CM_pos_cols = [f"CM{i}_X" for i in range(1,numMu+1)] + \
                [f"CM{i}_Y" for i in range(1,numMu+1)] + \
                [f"CM{i}_Z" for i in range(1,numMu+1)]

    CM_vel_col = [f"CM{i}_XVel" for i in range(1,numMu+1)] +\
            [f"CM{i}_YVel" for i in range(1,numMu+1)] +\
            [f"CM{i}_ZVel" for i in range(1,numMu+1)]

    input_cols = input_cols + CM_pos_cols + CM_vel_col


    model = MLP_DT_Complex2(input_size=33, output_size=1).to(device)
    ...
    input_scaler = joblib.load(f"./{model_folder}/ICRA2025_MLP_CAP_input_scaler{model_id}.pkl")
    # output_scaler = joblib.load(f"./{model_folder}/ICRA2025_MLP_CAP_output_scaler{model_id}.pkl")
...
class Tethernet_Env(gym.Env):
    def __init__(self, port, env_id, num_envs):
        ...
        # Weight parameters
        self.alpha1 = 1.5       # Rcenter
        self.alpha2 = 1.0       # Rfuel
        self.alpha3 = 1.0       # Rarea
        self.alpha4 = 1.0       # Ropen
        self.alpha5 = 3.0       # Rphase
        self.alpha6 = 3.0       # Rfin
    ...
    def get_random_row(self, env_id, iteration, num_envs, total_rows):
        ...
        # random.seed(seed)
        row_number = np.random.permutation(total_rows-1)[int(env_id)]
    ...
    def calculate_reward(self, full_traj, target_ground_truth_pose, target_states_est, action6):
        ...
        Rphase = self.alpha5*Rphase


        xMU_t_open = xMU[t_open]
        yMU_t_open = yMU[t_open]
        zMU_t_open = zMU[t_open]
        xMU_pre = xMU[t_open - 10]
        yMU_pre = yMU[t_open - 10]
        zMU_pre = zMU[t_open - 10]
        
        vel_x = xMU_t_open - xMU_pre
        vel_y = yMU_t_open - yMU_pre
        vel_z = zMU_t_open - zMU_pre
        mlp_input = self.target_ground_truth[:3] + self.target_angular_velocity + self.target_ground_truth[3:6] + \
                    xMU_t_open.tolist() + yMU_t_open.tolist() + zMU_t_open.tolist() + \
                    vel_x.tolist() + vel_y.tolist() + vel_z.tolist()
        capture_result = run_MLP(mlp_input)
        if capture_result==1:
            Rfin = 1
        else:
            Rfin = 0

        Rfin = self.alpha6 * Rfin

        total_reward = Rcenter + Rfuel + Rarea + Ropen + Rphase + Rfin
        ################# END OF NEW REWARD ##################
        ...
        # Write a newline character to the file
            act_rw =  np.asarray([Rcenter, Rfuel, Rarea, Ropen, Rphase, Rfin, total_reward, 

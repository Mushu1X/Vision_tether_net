...
def load_image_gt(dataset, config, image_id):
    ...
    if config.ROT_AUG or config.ROT_IMAGE_AUG:
        ...
        assert config.ORIENTATION_PARAM == 'euler_angles'
    ...
    while True:
        try:
            ...
            if 'image_id' in locals():
                ...
                logging.exception(f"Error occurred before image_id was set: {str(e)}")
            error_count += 1
            if error_count > 5:
                raise
...
class UrsoNet():
    ...
    def softmax_loss_graph(self, y_gt, y_pred):
        ...
        loss = tf.keras.losses.categorical_crossentropy(y_gt, y_pred, from_logits=True)
    ...
    def load_weights(self, weights_in_path, weights_out_path, by_name=False, exclude=None):
        ...
        if by_name:
            keras_model.load_weights(weights_in_path, by_name=True, skip_mismatch=True)
        else:
            keras_model.load_weights(weights_in_path, by_name=False, skip_mismatch=True)
    ...
    #             total_loss += loss
    #             # Add loss to the layer instead of the model
    #             layer.add_loss(loss)
    #         return total_loss
    ...
    #     for w in self.keras_model.trainable_weights:
    #         if 'gamma' not in w.name and 'beta' not in w.name:
    #             l2_reg_loss += tf.keras.regularizers.l2(self.config.WEIGHT_DECAY)(w)
        
    #     # Add L2 regularization loss to the model
    #     self.keras_model.add_loss(lambda: l2_reg_loss)

    #     # Compile
    #     self.keras_model.compile(
    #         optimizer=optimizer,
    #         loss=custom_loss,
    #         metrics=loss_names  # This will track individual losses as metrics
    #     )

    from tensorflow.python.keras.engine.keras_tensor import KerasTensor  # Ensure this import is correct

    # def compile(self, learning_rate, momentum):
    #     """Prepares the model for training by adding losses, regularization, and
    #     metrics, then calls the Keras compile() function.
    #     """
    ...
    #     # Clear any previous losses
    #     self.keras_model._losses = []
    #     self.keras_model._per_input_losses = {}

    #     # Determine loss names based on config
    #     if self.config.REGRESS_KEYPOINTS:
    #         loss_names = ["loc_loss", "k2_loss", "k3_loss"]
    #     else:
    #         loss_names = ["loc_loss", "ori_loss"]
    ...
    #         for name in loss_names:
    #             layer = self.keras_model.get_layer(name)
    #             loss = tf.reduce_mean(layer.output) * self.config.LOSS_WEIGHTS.get(name, 1.0)
    #             total_loss += loss
    #         return total_loss

    #     # Add L2 Regularization
    #     reg_losses = [
    #         tf.keras.regularizers.l2(self.config.WEIGHT_DECAY)(w) for w in self.keras_model.trainable_weights
    #         if 'gamma' not in w.name and 'beta' not in w.name
    #     ]

    #     # Use a lambda function to defer the addition of the regularization loss
    #     self.keras_model.add_loss(lambda: tf.add_n(reg_losses))

    #     # Compile the model with the optimizer and the custom loss function
    #     self.keras_model.compile(
    #         optimizer=optimizer,
    #         loss=custom_loss,
    #         metrics=[tf.keras.metrics.Mean(name=name) for name in loss_names]  # Track losses as metrics
    #     )
    ...
    #             # Check if layer.output is a list and convert to tensor if needed
    #             tensor = layer.output
    #             if isinstance(tensor, (list, tuple)):
    #                 # Assuming you want to use the first element of the list if it's a list
    #                 tensor = tensor[0]
    #             # Adjust the check to include both tf.Tensor and KerasTensor
    #             if not isinstance(tensor, (tf.Tensor, KerasTensor)):
    #                 print(f"Layer output is not a standard tensor, but treating as a scalar tensor: {tensor}")
    #                 continue  # This will treat scalar tensors appropriately
    #             self.keras_model.add_metric(tf.keras.layers.Lambda(lambda x: tf.reduce_mean(x))(tensor), name=name, aggregation='mean')

    def compile(self, learning_rate, momentum):
        """Prepares the model for training by adding losses, regularization, and metrics, then calls the Keras compile() function."""
        ...
        self.keras_model.compile(
            optimizer=optimizer,
            loss=[None] * len(self.keras_model.outputs),  # Placeholder for Keras API compatibility
        )

        # Add metrics for monitoring
        for name in loss_names:
            layer = self.keras_model.get_layer(name)
            metric_tensor = tf.reduce_mean(layer.output)
            if not isinstance(metric_tensor, tf.Tensor):
                # Convert KerasTensor to a standard TensorFlow tensor
                metric_tensor = tf.convert_to_tensor(metric_tensor)
            self.keras_model.add_metric(metric_tensor, name=name, aggregation='mean')
